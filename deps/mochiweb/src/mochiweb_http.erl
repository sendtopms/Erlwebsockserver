...
%%%%%%%%%%%%%%%%%%%%%% Websocket Enabled %%%%%%%%%%%%%%%%%
%% modified-by: Senthilkumar Peelikkampatti (sendtopms@gmail.com)
%% for enabling Websocket 
%% loop/2 and after_response/2 got modified to adapt websocket protocol 
%%%%%%%%%%%%%%%%%%%%%% Websocket Enabled %%%%%%%%%%%%%%%%%
...
loop(Socket, Body0) ->
	Body= websocket_utils:adapt_body(Body0),
...
request(Socket, Body) ->
	case gen_tcp:recv(Socket, 0, ?IDLE_TIMEOUT) of
		{ok, {http_request, Method, Path, Version}} ->
			io:format("~p 1request http_request is ~p~n", [?MODULE, Method]),
			headers(Socket, {Method, Path, Version}, [], Body, 0);
		{error, {http_error, "\r\n"}} ->
			io:format("~p 2request http_error is ~p~n", [?MODULE,http_error]),
			request(Socket, Body);
		{error, {http_error, "\n"}} ->
			request(Socket, Body);
		_Other ->
			io:format("~p _Other1 request is ~p~n", [?MODULE, _Other])
	...
	case gen_tcp:recv(Socket, 0, ?IDLE_TIMEOUT) of
		...
		{ok, {http_header, _, Name, _, Value}} ->
			headers(Socket, Request, [{Name, Value} | Headers], Body,
					1 + HeaderCount);
		_Other ->
		io:format("headers and  gen_tcp:close(Socket) ~n"),
		gen_tcp:close(Socket),
			exit(normal)
	end.
...
after_response(Body, Req) ->
	...
	case Req:should_close() of
		true ->
			...
			exit(normal);
		false ->
			case websocket_utils:is_websocket_request(Req) of
				true -> 
%% 					Req:cleanup(),
					io:format("Moving to Websocket Mode [after_response] ~n"),
				  erlwebsockserver:hshake(Socket);
				false -> 
					Req:cleanup(),
